<?php
	...
	if($_GET['evolucao_mes'] != '' && $_GET['evolucao_ano'] != ''){
	    ...
	    $sql_evolucao = "SELECT * FROM estudo_caso_evolucao WHERE estudo_caso = {$_GET['id']} AND ano = {$_GET['evolucao_ano']} AND mes = {$_GET['evolucao_mes']} LIMIT 1";
	    $stmt_evolucao = $conn->query($sql_evolucao);
        $registros_evolucao = $stmt_evolucao->num_rows;
        ...
        /* Se encontrar o PAS */
        if ($registros_pas > 0) {
        	if ($registros_evolucao > 0) {
        	$e = '<html>
			<head>
				<meta charset="utf-8">
				<title>Estudo de Caso - Evolução das Metas e Demandas</title>
            ...
            $e .= "<div id='head'></div>";
            $e .= "<h2 align='center' style='margin-top: 40px;'>Estudo de Caso - Evolução das Metas e Demandas</h2> <br />";
			...
			$e .= '</body></html>';
	        	...
	        	echo "Evolução de <strong>".$_GET['evolucao_mes']."/".$_GET['evolucao_ano']."</strong> não encontrada!";
	        }
        } else {
        ...
        /* Exibe */
        $dompdf->stream(
            "Estudo de Caso - Evolução.pdf", /* Nome do arquivo de saída */
            array(
