<?php
	...
	if(isset($_GET['acolhimento']) && $_GET['acolhimento'] != ''){
        ...
        if ($registros_acolhimento > 0) {
			...
			<head>
				<meta charset="utf-8">
				<title>Comunicado de Acolhimento - ' . strtoupper($acolhido["nome"]) . '</title>
			...
			$e .= "<p>
                ...
                <strong>Assunto: <u>COMUNICADO DE ACOLHIMENTO</u></strong> <br />
                Prezados, <br />
                Informo a vossa senhoria o acolhimento, conforme discriminado a seguir, referente ao mês de ". $nome_mes .".
            </p>";
            ...
            $e .= "<th align='center'>Acolhimento na CT</th>";
            $e .= "</tr>";
            $e .= "<tr>";
            $e .= "<td align='center'>". $acolhido['nome'] ."</td>";
            $e .= "<td align='center'>". $acolhido['rg'] ."</td>";
            $e .= "<td align='center'>". date('d/m/Y', strtotime($acolhimento['data_inicio'])) ."</td>";
            $e .= "</tr>";
            $e .= "</table>";
            ...
            $e .= "<tr>";
            $e .= "<th colspan='".$registros_convenios."'>Forma de acolhimento</th>";
            $e .= "</tr>";
            $e .= "<tr>";
            // if($acolhimento['tipo_acolhimento'] == 0){
                while($convenios = $stmt_convenios->fetch_assoc()){
                    if($acolhimento['convenio'] == $convenios['id']){
                        $e .= "<td>( X ) ". $convenios['nome'] ."</td>";
                    } else {
                        $e .= "<td>( ) ". $convenios['nome'] ."</td>";
                    }
                }
            // }
        ...
        /* Exibe */
        $dompdf->stream(
            "COMUNICADO DE ACOLHIMENTO - " . strtoupper($acolhido["nome"]) . ".pdf", /* Nome do arquivo de saída */
            array(
