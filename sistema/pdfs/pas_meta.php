...
// ini_set('display_errors', 1);
	...
	if($_GET['meta_mes'] != '' && $_GET['meta_ano'] != ''){
	    ...
	    $sql_meta = "SELECT * FROM pas_metas WHERE pas = {$_GET['id']} AND ano = {$_GET['meta_ano']} AND mes = {$_GET['meta_mes']} LIMIT 1";
        ...
        if ($registros_pas > 0) {
			...
			<head>
				<meta charset="utf-8">
				<title>Plano de Atendimento Singular - Meta</title>
            ...
            $e .= "<div id='head'></div>";
            $e .= "<h2 align='center' style='margin-top: 40px;'>Plano de Atendimento Singular - Meta</h2> <br />";

	        $e .= '<strong>Áreas na vida do acolhido para serem trabalhadas no mês de: </strong>' . $nome_mes . '/' . $meta['ano'] . '<br />';

	        $e .= '<br /><br /><br />';
	        
	        $e .= '<img src="../public/uploads/acolhimentos/pas/'.$meta['pas'].'/metas/'.$meta['arquivo'].'" width="100%" />';
